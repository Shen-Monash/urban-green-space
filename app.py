...
def plants():
    ...
    if not request.url.find("?")<0:
        ...
        if request.args.get('Type') and request.args.get('Type') != 'all':
            if request.args.get('Sunshine') and request.args.get('Sunshine') != 'all':
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_type,plant_sun_reqs,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_type,plant_sun_reqs,plant_water_freqs))
                else:
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_type,plant_sun_reqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_type,plant_sun_reqs))
            else:
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_type,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_type,plant_water_freqs))
                else:
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_type))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_type))
        else:
            if request.args.get('Sunshine') and request.args.get('Sunshine') != 'all':
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_sun_reqs,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s and plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_sun_reqs,plant_water_freqs))
                else:
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_sun_reqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_sun_reqs))
            else:
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy",(plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select plant.plant_id,plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy,group_concat(location separator ' and ') as location from plant LEFT OUTER JOIN plant_location on plant.plant_id = plant_location.plant_id where plant_water_freqs = %s GROUP BY plant.plant_id, plant_latin_name,plant_common_name,plant_family,plant_desc,plant_type,plant_size,plant_maint_amount,plant_sun_reqs,plant_water_freqs,plant_soil_reqs,plant_allergy" + pag['limit'],(plant_water_freqs))
                else:
