...
def plants():
    ...
    if not request.url.find("?")<0:        
        if request.args.get('Name') and request.args.get('Name') != '':
            ...
            cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_latin_name = %s or plant_common_name = %s",(plant_name,plant_name))
            pag = pages(len(cur.fetchall()), 8)
            cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_latin_name = %s or plant_common_name = %s" + pag['limit'],(plant_name,plant_name))
            result = cur.fetchall()
            db.close()
            cur.close()
