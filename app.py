...
def plants():
    ...
    if not request.url.find("?")<0:
        if request.args.get('Name') and request.args.get('Name') != '':
            ...
            cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_latin_name like %s or plant_common_name like %s",(plant_name,plant_name))
            pag = pages(len(cur.fetchall()), 8)
            cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_latin_name like %s or plant_common_name like %s" + pag['limit'],(plant_name,plant_name))
            result = cur.fetchall()
            db.close()
            cur.close()
        ...
        if request.args.get('Type') and request.args.get('Type') != 'all':
            if request.args.get('Sunshine') and request.args.get('Sunshine') != 'all':
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s and plant_water_freqs = %s",(plant_type,plant_sun_reqs,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s and plant_water_freqs = %s" + pag['limit'],(plant_type,plant_sun_reqs,plant_water_freqs))
                else:
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s",(plant_type,plant_sun_reqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_sun_reqs = %s" + pag['limit'],(plant_type,plant_sun_reqs))
            else:
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_water_freqs = %s",(plant_type,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s and plant_water_freqs = %s" + pag['limit'],(plant_type,plant_water_freqs))
                else:
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s",(plant_type))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_type = %s" + pag['limit'],(plant_type))
        else:
            if request.args.get('Sunshine') and request.args.get('Sunshine') != 'all':
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s and plant_water_freqs = %s",(plant_sun_reqs,plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s and plant_water_freqs = %s" + pag['limit'],(plant_sun_reqs,plant_water_freqs))
                else:
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s",(plant_sun_reqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_sun_reqs = %s" + pag['limit'],(plant_sun_reqs))
            else:
                if request.args.get('Water') and request.args.get('Water') != 'all':
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_water_freqs = %s",(plant_water_freqs))
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id where plant_water_freqs = %s" + pag['limit'],(plant_water_freqs))
                else:
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id")
                    pag = pages(len(cur.fetchall()), 8)
                    cur.execute("select * from plant Left Join plant_location on plant.plant_id = plant_location.plant_id" + pag['limit'])
        result = cur.fetchall()
        db.close()
        cur.close()
        return render_template('plants.html', data=result, pag=pag)
